...
<head>
  <meta charset="UTF-8">
  <title>Oral Translation Result</title>
  <style>
    body {
  ...
  <script>
    ...
    function speak(index) {
      ...
      const hindi = new SpeechSynthesisUtterance("Hindi sentence: " + data.hindi);
      const user = new SpeechSynthesisUtterance("Your translation: " + data.user);
      const correct = new SpeechSynthesisUtterance("Correct answer: " + data.correct);

      hindi.voice = window.hindiVoice;
      user.voice = window.engVoice;
...
<div class="container">
  ...
  <h2>Oral Translate to English - <?= htmlspecialchars($user) ?></h2>
